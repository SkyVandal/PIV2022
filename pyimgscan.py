...
from cvtools import get_outlines
...
def pre_process(img):
    ...
    # calculate the ratio of the image to the new height, so it can be scaled back to the original size later
    scale = img_adj.shape[0] / 500.0

    # scale the image height down
...
def get_hull(img_edge):
    # make a copy of the edge image because the following function manipulates the input
    img_temp_hull = img_edge.copy()

    # find outlines in the (newly copied) edge image
    outlines = get_outlines(img_temp_hull)

    # create a blank image for convex hull operation
    img_hull = blank(img_temp_hull.shape, img_temp_hull.dtype, "0")
...
def get_corners(img_hull):
    # make a copy of the edge image because the following function manipulates the input
    img_outlines = img_hull.copy()

    # find outlines in the convex hull image
    outlines = get_outlines(img_outlines)

    # sort the outlines by area from large to small, and only take the largest 4 outlines
    ...
    for outline in outlines:
        ...
        # approximate a rough contour for each outline found, with (hopefully) 4 points (rectangular sheet of paper)
        # Douglas-Peuker Algorithm
        # FIRST OPTION is the input outline;
        # SECOND OPTION is the accuracy of approximation (epsilon), set to a percentage of the perimeter of the outline
        # THIRD OPTION is whether to assume an outline is closed, which in this case is (sheet of paper)
        approx = cv2.approxPolyDP(outline, 0.02 * perimeter, True)

        # if the approximation has 4 points, then assume it is correct, and assign points to the 'corners' variable
        if len(approx) == 4:
            corners = approx
            break
...
# obtain the adjusted image, scaled image along with its scale factor, and the canny edge image
img_adj, scale, img_scaled, img_edge = pre_process(img)

# perform convex hull on edge image to prevent incomplete outline
img_hull = get_hull(img_edge)

# obtain 4 corner points of the convex hull image
corners = get_corners(img_hull)

# scale the corner points back to the original size of the image using the scale calculated previously
corners = corners.reshape(4, 2) * scale

# finally correct the perspective of the image by applying four-point perspective transform
